...
public final class FireRedASR2Model: Module {
    ...
    func generate(
        ...
        maxLen: Int = 0,
        graph: HotwordGraph? = nil,
        check: () throws -> Void = {}
    ) throws -> FireRedOutput {
        try check()
        ...
        try check()
        let (sequence, confidenceScores) = try ContextualBeamSearch.decode(
            encoderOutput: encoderOutput, decoder: decoder, graph: graph,
            beamSize: beamSize,
            maxLen: maxLen > 0 ? maxLen : encoderOutput.shape[1],
            smoothing: softmaxSmoothing,
            lengthPenalty: lengthPenalty,
            eosPenalty: eosPenalty, check: check
        )
        ...
        return FireRedOutput(text: text, tokens: trimmedSequence.map(Int.init),
                             confidence: roundedConfidence,
                             truncated: !sequence.contains(Int32(config.eosID)))
    }

    private func loadAssets(from modelDirectory: URL) throws {
        let data = try Data(contentsOf: modelDirectory.appendingPathComponent("cmvn.json"))
        let cmvn = try JSONDecoder().decode(FireRedASR2CMVN.self, from: data)
        guard cmvn.means.count == config.idim, cmvn.istd.count == config.idim,
              cmvn.means.allSatisfy(\.isFinite), cmvn.istd.allSatisfy({ $0.isFinite && $0 > 0 }) else {
            throw LocalASRError.invalidInput("Invalid FireRed normalization statistics.")
        }
        cmvnMeans = MLXArray(cmvn.means)
        cmvnIstd = MLXArray(cmvn.istd)
        tokenizer = try FireRedASR2Tokenizer(modelDirectory: modelDirectory)
        guard vocabulary.count == config.odim else {
            throw LocalASRError.invalidInput("FireRed vocabulary size does not match its model.")
        }
    }
